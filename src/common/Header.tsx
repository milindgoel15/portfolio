...
let Header = (): JSX.Element => {
	...
	let renderThemeChanger = (
		<ThemeContext.Provider
			value={{ systemTheme, theme, setTheme, mounted, setMounted }}
		>
			<ThemeChanger />
		</ThemeContext.Provider>
	);
		...
		<>
			<header className="max-w-7xl mx-auto px-8 md:mx-8 flex xl:mx-auto h-24 items-center justify-between">
				...
				<nav className="flex gap-6">
					...
					<ul
						onClick={() => setNavBarOpen(false)}
						className={`md:flex md:items-center z-[1] text-black dark:text-white text-center md:z-auto md:static absolute w-full left-0 md:w-auto dark:bg-secondary md:dark:bg-transparent backdrop-blur md:backdrop-blur-none md:py-0 space-x-4 transition-all ease-in duration-400 ${
